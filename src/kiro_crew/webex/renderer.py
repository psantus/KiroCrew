...
class WebexRenderer(Renderer):
    ...
    async def _send_uploads(self, files: list[Any]) -> list[Any]:
        ...
        for item in files:
            ...
            # is the model-authored alt text of the markdown image reference — an
            # untrusted egress surface like any other. It bypasses the answer
            # body's redaction (that ran on the text these files were extracted
            # OUT of), so a delimiter-split credential in the alt (`AKIA**…**`,
            # whole once Webex renders the markup away) would otherwise ship. The
            # filename is our own temp-file basename, but the caption is the same
            # display-form floor every other ``_reply``/frame passes through.
            caption = webex_display_safe(item.alt or os.path.basename(item.path))
