        ...
        while True:
            ...
            async with self._queue.lock:
                while True:
                    ...
                    if item is None:
                        break
                    item_room = str(item[2].get("webex_room_id") or "")
                    if room is None:
                        room = item_room
                        email = str(item[2].get("webex_person_email") or "")
                    # Collapse only messages from the SAME room. One combined turn
                    # gets one envelope, so mixing rooms would answer one person's
                    # text into another's chat -- reachable whenever a unified key
                    # puts two humans on one queue. A different room defers itself
                    # AND everything behind it, so FIFO stays exact and the outer
                    # loop drains it next as its own turn in its own envelope.
                    if len(texts) < _MAX_COLLAPSE and item_room == room:
                        texts.append(item[1])
            ...
            drained = replace(
                ...
                room_id=room or inbound.room_id,
