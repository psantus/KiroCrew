...
class TestTheWriteIsLockedAndOffLoop:
    ...
    async def test_the_write_is_dispatched_through_run_config_write(self, wired):
        ...
        assert seen["kwargs"] == {
            ...
            "values": {"a": "1"},
            "removals": [],
            # None on a per-key write: `set`/`delete` name their own keys, so there is
            # no whole-scope baseline to compare and nothing it could lose.
            "expect": None,
        }
...
class TestAppTokensCannotReachTheVariablesEndpoint:
    """An app manifest can allowlist an API path, so reachability is not a reason to
    assume a caller is the operator.

    Both verbs are refused, and each direction is its own breach: GET discloses every
    operator variable at every scope, PUT rewrites what expands into the operator's own
    next prompt. Sibling handlers (`computer_use`, `source_providers`, `feedback`)
    already reject app callers this way; this endpoint did not.

    Distinct from `request_is_operator`, which decides whether text may EXPAND. That is
    a different question from whether this caller may touch the store at all.
    """

    async def test_get_is_refused(self, wired):
        req = _request("GET")
        req["app"] = "some-app"
        resp = await vh.api_variables(req)
        assert resp.status == 403
        assert json.loads(resp.text)["code"] == "app_forbidden"

    async def test_put_is_refused(self, wired):
        req = _request("PUT", {"scope": "global", "values": {"a": "1"}})
        req["app"] = "some-app"
        resp = await vh.api_variables(req)
        assert resp.status == 403

    async def test_the_refusal_happens_before_the_read(self, wired):
        """A 403 whose body still carried the cascade would disclose exactly what the
        refusal exists to protect."""
        req = _request("GET")
        req["app"] = "some-app"
        resp = await vh.api_variables(req)
        body = resp.text
        assert "effective" not in body and "winning_scope" not in body

    async def test_the_operator_is_unaffected(self, wired):
        """An absent app claim is the dashboard operator, which must still work."""
        resp = await vh.api_variables(_request("GET"))
        assert resp.status == 200
        assert "effective" in resp.text


class TestAnUntrustedStoreIsAnAnsweredRefusalNotACrash:
    """A security refusal that escapes as a 500 is a worse outcome than the refusal.

    `patch_store` raises `UntrustedStoreLocation` when the store directory is a link.
    Uncaught, aiohttp turns that into a bare 500 with no `code` -- so the operator sees
    "internal error", retries, and it never succeeds, because the fix is removing a
    symlink by hand and nothing told them that.

    409 rather than 500 on purpose: nothing is broken in the service. The on-disk state
    conflicts with what the store requires.
    """

    async def _put_through_a_linked_store(self, tmp_path, monkeypatch):
        from kiro_crew import platform_compat

        (tmp_path / "config.json").write_text("{}", encoding="utf-8")
        monkeypatch.setattr(cfg_loader, "config_path", lambda: tmp_path / "config.json")
        elsewhere = tmp_path / "agent-writable"
        elsewhere.mkdir()
        platform_compat.symlink_or_junction(str(elsewhere), str(tmp_path / "variables"))
        vstore.invalidate_cache()
        return await vh.api_variables(_request("PUT", {"scope": "global", "set": {"a": "1"}}))

    async def test_it_answers_409_with_a_code(self, tmp_path, monkeypatch):
        resp = await self._put_through_a_linked_store(tmp_path, monkeypatch)
        assert resp.status == 409
        assert json.loads(resp.text)["code"] == "store_untrusted_location"

    async def test_the_message_names_the_operator_action(self, tmp_path, monkeypatch):
        """The remedy is outside the agent's reach, so the body has to say what it is."""
        resp = await self._put_through_a_linked_store(tmp_path, monkeypatch)
        assert "symlink" in json.loads(resp.text)["error"].lower()
...
class TestBulkEdit:
    ...
    async def test_a_value_another_writer_changed_refuses(self, wired):
        """The baseline compares VALUES, not just key presence.

        Tab B edits one value; Tab A's editor still shows the old one. A key-only
        compare passes, and Tab A's stale text silently reverts Tab B's edit — a lost
        update, not the deliberate overwrite bulk edit is for. This case previously
        ...
        assert resp.status == 409
        assert json.loads(resp.body)["code"] == "variables_stale_bulk_base"
        assert _doc(store)["global"] == {"a": "changed-by-tab-b"}, "the write landed"

    async def test_an_unchanged_scope_still_applies(self, wired):
        """The positive control: a baseline that matches is not refused."""
        _, store = wired
        _seed(store, {"global": {"a": "1"}})
        resp = await vh.api_variables(
            _request("PUT", {"scope": "global", "bulk": "a=mine\n", "base": {"a": "1"}})
    ...
    async def test_the_compare_happens_inside_the_locked_mutation(self, wired):
        """Not before it. A check on a read taken outside the lock is a TOCTOU: two
        applies can both read the same `current`, both agree with their own baseline,
        and the second still clobbers the first."""
        handler = inspect.getsource(vh.api_variables)
        assert "expect=expect" in handler, "the baseline is not handed to the locked write"
        assert "_scope_pairs" not in handler, (
            "the handler reads the scope before the lock again; removals and the "
            "compare must both come from the baseline the lock verifies"
        )
        assert "StaleBaseline" in handler, "a stale baseline no longer surfaces as 409"
    ...
    async def test_a_malformed_baseline_is_refused(self, wired):
        for bad in ("not-an-object", [1, 2], {"a": 1}):
            resp = await vh.api_variables(
                _request("PUT", {"scope": "global", "bulk": "a=1", "base": bad})
            )
            assert resp.status == 400
            assert json.loads(resp.body)["code"] == "variables_invalid_values"
        ...
        await vh.api_variables(
            _request(
                "PUT",
                {
                    "scope": "global",
                    "bulk": "a=1\n",
                    "base": {"a": "1", "added_by_someone_else": "2"},
                },
            )
        )
        assert _doc(store)["global"] == {"a": "1"}
    ...
    async def test_an_empty_paste_clears_the_scope(self, wired):
        ...
        _seed(store, {"global": {"a": "1"}})
        resp = await vh.api_variables(
            _request("PUT", {"scope": "global", "bulk": "", "base": {"a": "1"}})
        )
        assert resp.status == 200
    ...
    async def test_bulk_cannot_be_combined_with_set_or_delete(self, wired):
        ...
        for extra in ({"set": {"a": "1"}}, {"delete": ["a"]}):
            resp = await vh.api_variables(
                _request("PUT", {"scope": "global", "bulk": "a=1", "base": {}, **extra})
            )
            assert resp.status == 400
    ...
    async def test_bulk_must_be_a_string(self, wired):
        resp = await vh.api_variables(
            _request("PUT", {"scope": "global", "bulk": {"a": "1"}, "base": {}})
        )
        assert resp.status == 400
        assert json.loads(resp.body)["code"] == "variables_invalid_values"
    ...
    async def test_an_unknown_workspace_is_refused_before_the_write(self, wired):
        resp = await vh.api_variables(
            _request("PUT", {"scope": "workspace", "workspace": "nope", "bulk": "a=1", "base": {}})
        )
        assert resp.status == 400
