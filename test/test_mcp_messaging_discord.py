...
def test_every_slack_only_field_is_covered_by_the_refusal() -> None:
    ...
    ``channel_type`` + ``target_id`` are the cross-channel addressing pair, not
    Slack options: they ROUTE a send to a non-Slack transport rather than being
    dropped on one, so they are excluded from the Slack-only set by design. If a
    new schema field is neither core nor an addressing field nor Slack-only, this
    assertion fails until it is classified — which is the drift it guards against.
    """
